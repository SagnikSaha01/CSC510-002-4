...
def test_create_restaurant(client, mocker):
    ...
    new_restaurant_payload = {
        ...
        "menu_items": [
            {"name": "Test Dish 1", "price": 10.99, "category": "Test"}
        ]
    }
...
def test_get_restaurant_details(client, mocker):
    ...
    mock_data_dict = {
        ...
        "menu_items": [
            {"id": "m1m1m1", "name": "Margherita Pizza", "price": 14.50}
        ]
    }
    ...
    mock_chain = MagicMock()
    mock_chain.select.return_value.eq.return_value.execute.return_value = mock_response # <-- FIX
...
def test_get_restaurants_500_server_error(client, mocker):
    ...
    mock_chain = MagicMock()
    mock_chain.select.return_value.execute.side_effect = Exception("Simulated database connection error")
