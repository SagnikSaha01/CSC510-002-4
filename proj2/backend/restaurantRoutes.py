...
def create_restaurant():
    ...
    try:
        ...
        if not restaurant_response.data:
            raise Exception("Failed to create restaurant. Response: " + str(restaurant_response.error or "No data returned"))
        ...
        if menu_items_data:
            ...
            menu_response = supabase.table('menu_items') \
                                    .insert(menu_items_data) \
                                    .execute()
            
            if not menu_response.data:
                supabase.table('restaurants').delete().eq('id', new_restaurant_id).execute()
                raise Exception("Menu items failed to save. Restaurant creation was rolled back.")
...
def get_recommendations():
    try:
        ...
        if not restaurants_response.data:
            return make_response(jsonify({"error": "No restaurants available in database"}), 404)

        # 2. Call the AI service to do the heavy lifting
        recommendations = ai_service.get_ai_recommendations(mood_text, restaurants_response.data)
        
        return jsonify({"recommendations": recommendations})

    except json.JSONDecodeError as e:
        return make_response(jsonify({"error": "Failed to parse AI response", "details": str(e)}), 500)
    except Exception as e:
